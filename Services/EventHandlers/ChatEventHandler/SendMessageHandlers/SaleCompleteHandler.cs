...
public class SendMessageCompleteStrategy : IMessageStrategy
{
    public MessageType Target => MessageType.OfferCompleted;
    private readonly DbManager _db;
    private readonly IHasher _hasher;
    private readonly IMediator _mediator;
    private readonly ILogger<SendMessageCompleteStrategy> _logger;

    public SendMessageCompleteStrategy(IMediator mediator, DbManager db, IHasher hasher, ILogger<SendMessageCompleteStrategy> logger)
    {
        _db = db;
        _hasher = hasher;
        _mediator = mediator;
    ...
    public async Task<Result<MessageResponseDTO>> MessageHandler(int UserId, SendMessage request, CancellationToken cancellation)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result<MessageResponseDTO>.Failure("An error occurred while processing the message.", StatusCodes.Status500InternalServerError);
        }
    }
}