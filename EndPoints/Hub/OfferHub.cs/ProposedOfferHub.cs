...
{
    public async Task PropoesedOffer(ProposedItemDTO proposedItem)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                await Clients.Groups($"UserNotifcation_{result.Value!.ReceipientId}").SendAsync("NewMessageNotification", result.Value);
